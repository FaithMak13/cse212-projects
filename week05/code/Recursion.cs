...
public static class Recursion
{
    ...
    public static decimal CountWaysToClimb(int s, Dictionary<int, decimal>? remember = null)
    {
        ...
        //Use memoization to store the result for the current value of s
    ...
    public static void SolveMaze(List<string> results, Maze maze, int x = 0, int y = 0, List<ValueTuple<int, int>>? currPath = null)
    {
        ...
        var newPath = new List<(int, int)>(currPath);
        {
            ...
            foreach (var (dx, dy) in directions)
            {
                ...
                // Check if the move is valid and not already in the current path
                if (maze.IsValidMove(newPath, newX, newY) && !newPath.Contains((newX, newY)))
                {
                    SolveMaze(results, maze, newX, newY, newPath); //right
                    SolveMaze(results, maze, newX, newY, newPath); //down
                    SolveMaze(results, maze, newX, newY, newPath); //left
                    SolveMaze(results, maze, newX, newY, newPath); //up
                }
            }
        }
